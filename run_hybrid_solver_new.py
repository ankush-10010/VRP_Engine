...
def layer2_worker():
    """Background thread to re-optimize all routes periodically."""
    global current_routes, simulation_events, pending_orders
    while simulation_running:
        time.sleep(LAYER_2_INTERVAL_SECONDS)
        
        with state_lock:
            # We must pass the pending orders too, so Layer 2 can try to add them
            routes_to_optimize = {vid: r[:] for vid, r in current_routes.items()}
            pending_to_optimize = pending_orders[:]
            # --- NEW: Get a "before" set of pending IDs ---
            pending_ids_before = set(o['id'] for o in pending_to_optimize)

        if not any(routes_to_optimize.values()) and not pending_to_optimize:
            continue # Nothing to optimize

        print("\n--- [LAYER 2] Starting background fleet optimization... ---")
        
        # This new function re-optimizes *and* tries to assign pending orders
        optimized_routes, unassigned_orders = batch_optimization_vrp(
            routes_to_optimize,
            pending_to_optimize, # Give L2 the pending orders
            time_matrix,
            NUM_VEHICLES,
            VEHICLE_CAPACITY,
        ...
        # --- NEW: Find out what L2 assigned ---
        unassigned_ids_after = set(o['id'] for o in unassigned_orders)
        assigned_ids_by_l2 = pending_ids_before - unassigned_ids_after

        with state_lock:
            current_routes = optimized_routes
            # Any orders L2 couldn't assign go back into the pending list
            pending_orders = unassigned_orders 
            
            # Calculate simulation time for logging
            sim_seconds_elapsed = (datetime.now() - simulation_start_time).total_seconds()
            # This is tricky as we don't have a perfect link. Let's use a rough estimate.
            # This logic needs refinement to be truly accurate.
            # For now, let's just log the event.
            log_time = datetime.now().strftime("%I:%M:%S %p")
            # --- NEW: Log the orders L2 assigned ---
            if assigned_ids_by_l2:
                print(f"--- [LAYER 2] Successfully assigned {len(assigned_ids_by_l2)} pending orders. ---")
                for order_id in assigned_ids_by_l2:
                    # Find the full order object
                    order = next((o for o in pending_to_optimize if o['id'] == order_id), None)
                    if not order: continue

                    # Find which vehicle it ended up on
                    assigned_vehicle_id = -1
                    for v_id, route in optimized_routes.items():
                        if order_id in [o['id'] for o in route]:
                    ...
                    if assigned_vehicle_id != -1:
                        global_order_assignments_log.append({
                            ...
                            "order_id": order['id'],
                            "demand": order['demand'],
                            "location": all_locations[order['index']]['original_address'].split(',')[0],
            ...
            simulation_events.append({
                'type': 'optimization',
                'time': log_time,
                'description': f"[LAYER 2] Background optimization completed. {len(pending_orders)} orders remain pending."
            })
            
            print(f"--- [LAYER 2] Optimization complete. {len(pending_orders)} orders remain pending. ---")
...
def run_hybrid_simulation():
    ...
    try:
        ...
        print(f"✅ Master time matrix and {len(all_locations)} locations loaded.")
    except FileNotFoundError:
        print(f"FATAL Error: '{TIME_MATRIX_FILE}' not found. Run a 'build_master_matrix' script first.")
        return
